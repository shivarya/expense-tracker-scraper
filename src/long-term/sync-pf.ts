...
const INPUT_FILE = path.join(process.cwd(), 'data', 'enriched-pf.json');

async function syncPf() {
  console.log('🚀 Syncing PF long-term data\n');
  ...
  if (!Array.isArray(funds) || funds.length === 0) {
    throw new Error('No longTermFunds found in enriched-pf.json');
  }

  console.log(`📦 PF accounts to sync: ${funds.length}`);
  for (const f of funds) {
    console.log(`   ${f.account_number}: ₹${f.current_value.toLocaleString('en-IN')}`);
  }
